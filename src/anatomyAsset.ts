...
export function vertexWeights(
  ...
  const forearm = (): [number, number][] => {
    const chain = [a.lower, ...a.twists];
    const turn = THREE.MathUtils.clamp((2.42 - p.y) / .58, 0, 1) * 3;
    const segment = Math.min(2, Math.floor(turn));
    return mix(chain[segment], chain[segment + 1], turn - segment);
  };
  // Shoulder muscles cross anatomical attachment sites, not a horizontal
  // y-band. A hard cutoff at y=2.95 used to give adjacent vertices completely
  // different bones, stretching small triangles into visible tears.
  if (/deltoid/i.test(name))
    return mix(rig.torso, a.upper, smooth(.28, .47, Math.abs(p.x)));
  if (/infraspinatus|supraspinatus|subscapularis|teres (major|minor)/i.test(name))
    return mix(rig.torso, a.upper, smooth(.30, .44, Math.abs(p.x)));
  if (kind === "bone") {
    if (/^scapula[. ]/i.test(name)) return only(rig.torso);
    if (/^clavicle[. ]/i.test(name)) return only(rig.torso);
    if (/humerus/i.test(name)) return only(a.upper);
    if (/ulna/i.test(name)) return only(a.lower);
    if (/radius/i.test(name)) return only(a.twists[2]);
  ...
  if (family === "arm") {
    if (p.y > 2.46) {
      const attachment = smooth(2.965, 3.11, p.y) * (1 - smooth(.31, .47, Math.abs(p.x)));
      return mix(a.upper, rig.torso, attachment);
    }
    if (p.y > 2.38) {
      const upperWeight = smooth(2.38, 2.46, p.y);
      return [...forearm().map(([i, w]): [number, number] => [i, w * (1 - upperWeight)]), [index(a.upper), upperWeight]];
    }
    if (p.y > 1.94) return forearm();
    if (p.y > 1.77) {
      const lowerWeight = smooth(1.77, 1.94, p.y);
      return [...forearm().map(([i, w]): [number, number] => [i, w * lowerWeight]), [index(a.hand), 1 - lowerWeight]];
    }
  ...
  if (Math.abs(p.x) > 0.29 && p.y > 2.70 && p.y < 3.29)
    return mix(
      rig.torso,
      a.upper,
      smooth(0.29, 0.44, Math.abs(p.x)) * smooth(2.70, 2.85, p.y) * (1 - smooth(3.12, 3.29, p.y)),
    );
